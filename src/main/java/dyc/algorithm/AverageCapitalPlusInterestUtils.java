...
public class AverageCapitalPlusInterestUtils {
     ...
     * 等额本息计算获取还款方式为等额本息的每月偿还本金和利息
     * 
     * 公式：贷款本金=每月偿还本息×〔(1＋月利率)＾还款月数-1〕÷（月利率×(1＋月利率)＾还款月数〕）
     * @param monthIncome
     *            每月偿还本金和利息
     * @param yearRate
     *            年利率
     ...
     * @return 贷款本金, 整数
     */
    public static double getPerMonthPrincipalInvest(double monthIncome, double yearRate, int totalmonth) {
        double monthRate = yearRate / 12;
        BigDecimal invest = new BigDecimal(monthIncome)
                .multiply(new BigDecimal(Math.pow(1 + monthRate, totalmonth) - 1))
                .divide(new BigDecimal(monthRate * Math.pow(1 + monthRate, totalmonth)), 2, BigDecimal.ROUND_DOWN);
        return invest.setScale(0, BigDecimal.ROUND_UP).doubleValue();
    }
    ...
    public static void main(String[] args) {
        ...
        double perMonthPrincipalInvest = getPerMonthPrincipalInvest(perMonthPrincipalInterest, yearRate, month);
        System.out.println("等额本息---本金：" + perMonthPrincipalInvest);
