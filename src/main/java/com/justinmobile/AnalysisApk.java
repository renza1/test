...
public class AnalysisApk {
				...
				if (zipEntry.isDirectory()) {
					...
					if ("AndroidManifest.xml".equals(zipEntry.getName())) {
						try {
							...
							/*
							 * AXmlResourceParser parser=new
							 * AXmlResourceParser();
							 * parser.open(zipFile.getInputStream(zipEntry));
							 * while (true) { int type=parser.next(); if
							 * (type==XmlPullParser.END_DOCUMENT) { break; }
							 * switch (type) { case XmlPullParser.START_TAG: {
							 * for (int i=0;i!=parser.getAttributeCount();++i) {
							 * if
							 * ("versionName".equals(parser.getAttributeName(i)
							 * )){ st[0] = getAttributeValue(parser,i); }else
	...
	// private static String getAttributeValue(AXmlResourceParser parser,int
	// index) {
